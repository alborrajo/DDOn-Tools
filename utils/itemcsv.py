...
with open("items.csv", "w") as f:
    f.write("ItemId,QualityStars,IconNo,IconColNo\n")

    for consumable in itemlist["ConsumableList"]:
        f.write(f"{consumable['ItemId']},0,{consumable['IconNo']},{consumable['IconColNo']}\n")

    for material in itemlist["MaterialList"]:
        f.write(f"{material['ItemId']},0,{material['IconNo']},{material['IconColNo']}\n")

    for key_item in itemlist["KeyItemList"]:
        f.write(f"{key_item['ItemId']},0,{key_item['IconNo']},{key_item['IconColNo']}\n")

    for job_item in itemlist["JobItemList"]:
        f.write(f"{job_item['ItemId']},0,{job_item['IconNo']},{job_item['IconColNo']}\n")

    for special_item in itemlist["SpecialItemList"]:
        f.write(f"{special_item['ItemId']},0,{special_item['IconNo']},{special_item['IconColNo']}\n")

    for weapon in itemlist["WeaponList"]:
        base_item = itemlist["ItemEquipWeaponGroupList"][weapon["WeaponBaseId"]]
        f.write(f"{weapon['ItemId']},{weapon['QualityStars']},{base_item['IconNo']},{base_item['IconColNo']}\n")

    for armor in itemlist["ArmorList"]:
        base_item = itemlist["ItemEquipProtectorGroupList"][armor["ArmorBaseId"]]
        f.write(f"{armor['ItemId']},{armor['QualityStars']},{base_item['IconNo']},{base_item['IconColNo']}\n")

    for jewelry in itemlist["JewelryList"]:
        f.write(f"{jewelry['ItemId']},0,{jewelry['IconNo']},{jewelry['IconColNo']}\n")