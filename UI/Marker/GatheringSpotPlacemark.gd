...
func _ready() -> void:
	assert(gathering_spot.connect("changed", Callable(self, "_on_gathering_spot_changed")) == OK)
...
func _on_gathering_spot_changed() -> void:
	...
	for item in gatheringItems:
		...
		assert(item_placemark.connect("placemark_removed", Callable(self, "_on_item_removed").bind(item)) == OK)
