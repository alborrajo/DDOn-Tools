...
var stage_id: int: set = _set_stage_id
var npc_id: int: set = _set_npc_id
var institution_function_id: int: set = _set_institution_function_id
...
func get_display_name() -> String:
	...
	return tr('NPC_NAME_'+str(npc_id))+' - '+tr('FUNC_CLASS_NAME_'+str(institution_function_id))
...
func _set_shop(value: Shop) -> void:
	if value == null:
		return
	if shop != null:
		shop.disconnect("changed", Callable(self, "_on_shop_changed"))
	shop = value
	assert(shop.connect("changed", Callable(self, "_on_shop_changed")) == OK)
	_on_shop_changed()
...
func _on_shop_changed() -> void:
	...
	for good in shop.get_goods():
		...
		if not good.is_connected("changed", Callable(self, "_on_shop_item_changed")):
...
func _draw_tree_item(tree_item: TreeItem, rect: Rect2):
	...
	var color: Color = SelectedListManager.FILTER_NONMATCH_COLOR
	...
	var icon_position := Vector2(rect.position.x + (24 - float(good.item.icon.get_width())/2), rect.position.y + (float(rect.size.y)/2 - float(good.item.icon.get_height())/2))
...
func _on_ToggleButton_subgroup_selected():
	# Godot 4 migration
	# Function "_on_ToggleButton_subgroup_selected()" not found in base ToggleablePlacemark.
	# super._on_ToggleButton_subgroup_selected()
	for good in shop.get_goods():
