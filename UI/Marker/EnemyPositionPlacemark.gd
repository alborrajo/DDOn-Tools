...
var enemy_position: EnemyPosition: set = _set_enemy_position
...
func _on_enemy_position_changed() -> void:
	...
	if enemy_position != null: 
		for enemy in enemy_position.enemies:
			...
			assert(enemy_placemark.connect("placemark_removed", Callable(self, "_on_enemy_removed").bind(enemy)) == OK)
...
func _set_enemy_position(new_enemy_position: EnemyPosition) -> void:
	if enemy_position != null:
		if enemy_position.is_connected("changed", Callable(self, "_on_enemy_position_changed")):
			enemy_position.disconnect("changed", Callable(self, "_on_enemy_position_changed"))
		for enemy in enemy_position.get_enemies():
			if enemy_position.is_connected("changed", Callable(self, "_on_enemy_changed")):
				enemy_position.disconnect("changed", Callable(self, "_on_enemy_changed"))
	
	enemy_position = new_enemy_position
	
	if enemy_position != null:
		assert(enemy_position.connect("changed", Callable(self, "_on_enemy_position_changed")) == OK)

	_on_enemy_position_changed()

func _on_enemy_removed(enemy: Enemy) -> void:
	if enemy.is_connected("changed", Callable(self, "_on_enemy_changed")):
